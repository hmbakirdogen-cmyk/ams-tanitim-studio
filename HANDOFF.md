...
- **CILA TURU TAMAMLANDI (commit'li):** Cila-1 HUD braket + premium MPa/bar toggle + **zoom çarpışma fix** (kart min ölçü → yüksek zoomda çakışmaz) `228222b` · **yıldız tozu** (uzay derinliği) `67e18c4` · Cila-2 **kart mini grafik + KENDİ EKSENİ** (birim-duyarlı; basınç scala 0,6) + **40fps kare sınırı** (pervane) `075657b` · Cila-3 **cihaz OTO-bağlanma** (kılavuz açılınca otomatik arar + TEK cihazı seçer; çalışan köprü fonksiyonları yeniden kullanıldı, yeni bağlantı mantığı YOK) + flow kartı büyüt/ÜST-hizala (40px aşağı kaydı) + TOPLAM'da hava ikonu + basınç sağa/sıcaklık sola `793aceb` · **OPSİYONEL** Sakin Mod anahtarı (⚡ Sidebar; canvas 40→12fps + CSS animasyon durur; VERİ CANLI; `data/eco.ts` + `<html data-eco>`) + grafik 3D-hava (recessed konsol derinliği, saf CSS) `4f81864`. NOT: rakam-animasyonu (odometre) DENENDİ→REDDEDİLDİ ("benzin pompası"); HeroKPI truncate KORUNDU (küçük/zoom'da taşma önler). UI çatısı: "uzay konsolu AMA asla şımarıklık YOK — JP SMC".
- **PAKET TAZE + MASAÜSTÜNDE:** `Desktop\SMC-AMS-Tanitim.zip` (46.4 MB, sürüm **4f81864**, SW'siz, içerik doğrulandı: Baslat.bat + app + runtime/node.exe + köprü). Eski 19 Haz zip değiştirildi. GitHub Release adımı ATLANDI (dışa açık → Mehmet abi).
- **AÇIK — yalnız Mehmet abi/cihaza bağlı:** (1) AMS görseli fringe + kolaj kesme (görsel iş + onun gözü; körlemesine YAPILMADI — vitrini bozma riski), (2) cihaz oto-bağlanma GERÇEK cihaz testi (düğüm adları onayı), (3) GitHub auto-update Release + dal push (dışa açık → onayı). Her commit tsc 0 + build 0 + headless screenshot doğrulamalı.
