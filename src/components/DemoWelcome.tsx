      ...
      >
        <div className="grid grid-cols-1 items-center gap-7 lg:grid-cols-[1.05fr_1fr]">
          ...
          <div className="relative w-full overflow-hidden rounded-2xl border border-[var(--hair)] bg-[#0a1424]" style={{ aspectRatio: '1380 / 660' }}>
            <span className="absolute left-3 top-3 z-10 rounded-md px-2.5 py-1 text-[11px] font-bold text-white" style={{ background: '#0072CE', boxShadow: '0 4px 14px -4px rgba(0,114,206,0.9)' }}>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <div className="mt-1.5 text-sm text-[var(--ink-soft)]">{PRODUCT.brand} · {t('Tanıtım')}</div>
            <p className="mt-3 max-w-xs text-[13px] leading-snug text-[var(--ink-soft)]">
              {t('Hava Yönetim Sistemi’ni canlı verilerle keşfedin — kurulum gerekmez.')}
            </p>
            <motion.button
            ...
            >
              {t('Demo’ya Gir')} <ArrowRight size={18} />
            </motion.button>
          </div>
        </div>
