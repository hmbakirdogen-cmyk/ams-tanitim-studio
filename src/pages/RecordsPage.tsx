  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto pr-1">
      <PageHeader
        ...
        right={
          <button onClick={() => setAnalyze({ points: data.log, title: 'Şu Anki Oturum', startedAt: data.startedAt })} disabled={data.log.length < 2} className="flex items-center gap-1.5 rounded-lg border border-[var(--hair)] px-3 py-2 text-xs font-medium text-[var(--ink-soft)] transition hover:text-white disabled:opacity-40">
            <BarChart3 size={14} /> Şu anki oturumu analiz et / rapor
          </button>
        }
      />
          ...
          {list.map((rec) => (
            <div key={rec.id} className="glass flex items-center gap-4 rounded-2xl px-5 py-3.5">
              ...
              <div className="ml-auto flex shrink-0 items-center gap-2">
                <button onClick={() => setAnalyze({ points: rec.points, title: rec.name, startedAt: rec.startedAt })} title="Aralık analizi / rapor" className="flex items-center gap-1.5 rounded-lg border border-[var(--hair)] px-3 py-1.5 text-xs font-medium text-[var(--ink-soft)] transition hover:text-white"><BarChart3 size={14} /> Analiz</button>
